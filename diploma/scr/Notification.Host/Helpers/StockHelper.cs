...
namespace Notification.Host.Helpers
{
    public class StockHelper
    {
        ...
        {
            "Wheat", "Corn", "Sugar", "Soybean", "Soy oil", "Soy Meat", "OJ Future", "Coffee", "Cocoa", "Rice", "Oats",
            "Milk", "Cotton", "Lumber"
        };
        ...
        {
            "USD IDX Future", "USD/JPY Future", "GBP/USD Future", "AUD/USD Future", "DOW Future", "S&P Future",
            "NAS Future", "S&P MID MINI"
        };
        ...
        {
            var newData = new FinancialData[newCount];
            var rng = new Random();
            ...
            for (var i = 0; i < newCount; i++)
            {
                ...
                newData[i].OpenPriceDiff =
                    Math.Round((newData[i].OpenPrice - newData[i].Price) / newData[i].Price * 100 * 15, 4);
                ...
                newData[i].StartYDiff =
                    Math.Round((newData[i].StartY - newData[i].Price) / newData[i].Price * 100 * 15, 4);
                newData[i].HighYDiff =
                    Math.Round((newData[i].HighY - newData[i].Price) / newData[i].Price * 100 * 15, 4);
                newData[i].LowYDiff = Math.Round((newData[i].LowY - newData[i].Price) / newData[i].Price * 100 * 15, 4);
                newData[i].HighDDiff =
                    Math.Round((newData[i].HighD - newData[i].Price) / newData[i].Price * 100 * 15, 4);
        ...
        public static void UpdateRandomPrices(FinancialData[] finData)
        {
            var rnd = new Random();
        ...
        public static void UpdateAllPrices(FinancialData[] finData)
        {
            for (var i = 0; i <= finData.Length / 2; i++)
            {
        ...
        private static void RandomizeObjectData(FinancialData finDataObj)
        {
            var pc = GenerateNewPrice(finDataObj.Price);
        ...
        private static PercentChange GenerateNewPrice(double price)
        {
            var rnd = new Random();
            const int volatility = 2;

            var changePercent = 2 * volatility * rnd.NextDouble();
            if (changePercent > volatility)
            {
                changePercent -= 2 * volatility;
            }

            var changeAmount = price * (changePercent / 100);
            var newPrice = price + changeAmount;

            var result = new PercentChange
            {
                Price = Math.Round(newPrice, 4),
                ChangePercent = Math.Round(changePercent, 4) / 100
            };
        ...
        /// <returns></returns>
        private static FinancialData CopyObjectByValue(FinancialData obj)
        {
            var json = Newtonsoft.Json.JsonConvert.SerializeObject(obj);
            var newObj = Newtonsoft.Json.JsonConvert.DeserializeObject<FinancialData>(json);
